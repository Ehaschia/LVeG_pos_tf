...
def read_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, max_size=None,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        # bpos = Variable(torch.from_numpy(pid_inputs), volatile=volatile)
        ...
        # words = tf.constant(wid_inputs, name='words')
        # chars = tf.constant(cid_inputs, name='chars')
        # pos = tf.constant(pid_inputs, name='pos')
        # heads = tf.constant(hid_inputs, name='heads')
        # types = tf.constant(tid_inputs, name='types')
        # masks = tf.constant(masks, name='masks')
        # single = tf.constant(single, name='single')
        # lengths = tf.constant(lengths, name='lengths')
        ...
        data_variable.append((wid_inputs, cid_inputs, pid_inputs, hid_inputs, tid_inputs, masks, single, lengths))
...
def get_batch_variable(data, batch_size, unk_replace=0.):
    ...
    bucket_size = bucket_sizes[bucket_id]
    batch_size = min(bucket_size, batch_size)
    index = np.random.permutation(bucket_size)[:batch_size]
    # if words.is_cuda:
    #     index = index.cuda()
    ...
    if unk_replace:
        ...
        ones = np.ones((batch_size, bucket_length))
        noise = np.random.binomial(1, unk_replace, (batch_size, bucket_length))
...
def iterate_batch_variable(data, batch_size, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        indices = None
        if shuffle:
            indices = np.random.permutation(bucket_size)
            # if words.is_cuda:
...
type_alphabet = create_alphabets("/home/ehaschia/Code/LVeG_pos_tf/data/alphabets/pos_crf/toy2_conllx_data_test/",
...
data_train = read_data_to_variable(train_path, word_alphabet, char_alphabet, pos_alphabet,
                                   type_alphabet)
# pw_cnt_map, pp_cnt_map = store_cnt(data_train, word_alphabet, pos_alphabet)
num_data = sum(data_train[1])

data_dev = read_data_to_variable(dev_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
data_test = read_data_to_variable(test_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
...
for epoch in range(1, num_epochs + 1):
    ...
    for batch in range(1, num_batches + 1):
        word, _, labels, _, _, masks, lengths = get_batch_variable(data_train, batch_size,
                                                                   unk_replace=0.0)
    print(word)