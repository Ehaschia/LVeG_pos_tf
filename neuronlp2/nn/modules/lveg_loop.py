...
def dynamic_lveg(cell, inputs, sequence_length=None, initial_state=None,
  ...
  Args:
    ...
    position : the position of transition matrix
  ...
  """
    ...
    with vs.variable_scope(scope or "lveg") as varscope:
...
def _dynamic_rnn_loop(cell,
  ...
  Args:
    ...
    dtype: (optional) Expected dtype of output. If not specified, inferred from
      initial_state.
    position: use for select transion matrix
  ...
  """
    ...
    input_shape_mu = array_ops.shape(flat_input[0])
    time_steps = input_shape_mu[0]
    batch_size = _best_effort_input_batch_size(flat_input)
    ...
    """
        ...
        call_cell = lambda: cell(input_t, state, position=time)
