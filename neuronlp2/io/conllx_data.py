...
def read_data_to_numpy(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, max_size=None,
                          normalize_digits=True, symbolic_root=False, symbolic_end=False):
    ...
    for bucket_id in range(len(_buckets)):
        ...
        # words = tf.constant(wid_inputs)
        # chars = tf.constant(cid_inputs)
        # pos = tf.constant(pid_inputs)
        # heads = tf.constant(hid_inputs)
        # types = tf.constant(tid_inputs)
        # masks = tf.constant(masks)
        # single = tf.constant(single)
